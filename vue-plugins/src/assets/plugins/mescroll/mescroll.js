function MeScroll(mescrollId, options) {
	...
	MeScroll.prototype.extendDownScroll = function(optDown) {
		...
		MeScroll.extend(optDown, {
			...
			autoShowLoading: false, //如果设置auto=true,那么是否显示下拉刷新的进度; 默认false
			isLock: false, //是否锁定下拉刷新,默认false;
			isBoth: false, //下拉刷新时,如果滑动到列表底部是否可以同时触发上拉加载;默认false,两者不可同时触发
			offset: 80, //在列表顶部,下拉大于80px,松手即可触发下拉刷新的回调
			outOffsetRate: 0.2, //在列表顶部,下拉的距离大于offset时,改变下拉区域高度比例;值越接近0,高度变化越小,表现为越往下越难拉
			minAngle: 45, //触发下拉最少要偏移的角度(滑动的轨迹与水平线的锐角值),取值区间  [0,90];默认45度
			mustToTop: !isIOS, //是否必须滑到顶部才能下拉;-webkit-overflow-scrolling:touch是iOS专属样式,所以iOS默认false,其他为true
			...
			beforeLoading: function(mescroll, downwarp) {
				//准备触发下拉刷新的回调
				//如果return true,将不触发showLoading和callback回调; 常用来完全自定义下拉刷新, 具体用法参考【淘宝 v6.8.0】
				return false;
			},
	...
	MeScroll.prototype.extendUpScroll = function(optUp) {
		...
		MeScroll.extend(optUp, {
			...
			noMoreSize: 5, //如果列表已无数据,可设置列表的总数量要大于5条才显示无更多数据;避免列表数据过少,显示无更多数据不美观
			...
			loadFull: {
				use: false, //列表第一页数据不满屏,不足以滑动触发上拉加载时,是否自动加载下一页;默认false,因为可调高page.size
				delay: 500 //延时执行的毫秒数; 保证列表数据或占位图片已加载完成,下拉上拉的进度动画已执行完毕,此时高度计算准确
			},
			empty: {
			...
			showLoading: function(mescroll, upwarp) {
				//上拉加载中.. 
				//mescroll.upProgressDom.style.display = "block"; 不通过此方式显示,因为ios快速滑动到底部,进度条会无法及时渲染
	...
	MeScroll.prototype.initDownScroll = function() {
		...
		function touchstartEvent(e) {
			...
			if(me.optDown.mustToTop) {
				//如果列表必须滑动到顶部才能下拉,则记录此时第一个手指(鼠标)距离列表顶部的距离
				//移动端获取:e.touches[0].pageY; PC端获取:e.clientY
		...
		function touchmoveEvent(e) {
			//列表在顶部且不在加载中
			if(me.startTop != null && me.scrollDom.scrollTop <= 0 && !me.isDownScrolling 
				&& (!me.isUpScrolling || (me.isUpScrolling && me.optUp.isBoth)) && !me.optDown.isLock) {
				...
				if(moveY > 0) {
					...
					if(me.downHight < me.optDown.offset) {
						if(me.movetype != 1) {
							...
							//取消列表回弹效果,避免设置downwarp的height时界面抖动
	 ...
	 *结束上拉加载 meScroll.endSuccess(dataSize, systime);
	 * dataSize : 联网获取到的数据条数;用于判断列表是否还有数据.如果不传则仅隐藏加载中的状态;
	 * systime : 加载第一页数据服务器返回的时间 (可空); 防止用户翻页时,后台新增了数据从而导致下一页数据重复
	 */
